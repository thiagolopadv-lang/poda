...
from routes.dashboard import verificar_sessao          # reutiliza auth existente
...
async def api_comercial(_: None = Depends(verificar_sessao)):
    ...
    # Novos assinantes hoje (contador incremental)
    try:
        hoje = date.today().isoformat()
        novos_pro    = int(await rate_limiter.redis.get(
            f"poda:conv:new:pro:{hoje}") or 0)
        novos_equipe = int(await rate_limiter.redis.get(
            f"poda:conv:new:equipe:{hoje}") or 0)
        churn_hoje   = int(await rate_limiter.redis.get(
            f"poda:churn:total:{hoje}") or 0)
    except Exception:
...
<body>
<a class="back" href="/dashboard">← Voltar ao painel principal</a>
<h1>🌿 Poda — Inteligência Comercial</h1>
<p class="sub" id="ts">Carregando dados...</p>

<div class="grid" id="grid"><p class="spin">⏳ Carregando métricas...</p></div>
...
function fmt_ms(v){ return v==null ? '—' : v+'ms'; }
function fmt_brl(v){ return 'R$ '+v.toFixed(2).replace('.',','); }
function badge(v,ok,warn){ return v<=ok?'ok':v<=warn?'warn':'err'; }
...
function render(d){
  ...
  // Badge de latência
  function lat_badge(ms,modality){
  ...
  <div class="card">
    ...
    <div class="big">${fmt_brl(f.mrr)}<span style="font-size:1rem;color:var(--sub)">/mês</span></div>
    <div class="row" style="margin-top:12px"><span class="label">ARR projetado</span><span class="val">${fmt_brl(f.arr)}</span></div>
    <div class="row"><span class="label">Ticket médio</span><span class="val">${fmt_brl(f.ticket_medio)}</span></div>
    ...
    <div class="row"><span class="label">Churn hoje</span><span class="val" style="color:${f.churn_hoje>0?'var(--red)':'var(--text)'}">-${f.churn_hoje}</span></div>
  </div>

  <!-- Planos -->
  <div class="card">
    <h2>📊 Assinantes por Plano</h2>
    <div class="row"><span class="label">Free</span><span class="val">${pl.free}</span></div>
    <div class="row"><span class="label">Pro — R$19/mês</span><span class="val">${pl.pro}</span></div>
    <div class="row"><span class="label">Equipe — R$79/mês</span><span class="val">${pl.equipe}</span></div>
    <div class="row" style="margin-top:8px"><span class="label">Taxa pago/total</span>
      <span class="val">${pl.free+pl.pro+pl.equipe>0?((pl.pro+pl.equipe)/(pl.free+pl.pro+pl.equipe)*100).toFixed(1):0}%</span></div>
  </div>
  ...
  <div class="card">
    ...
    <div class="funnel">
      ${bar(funil.limite_atingido,'Atingiram<br>limite')}
      ${bar(funil.intent_upgrade,'Pediram<br>/planos')}
      ${bar(funil.cpf_informado,'CPF<br>informado')}
      ${bar(funil.pix_gerado,'PIX<br>gerado')}
      ${bar(funil.pago,'Pago ✓')}
    </div>
    <div class="row" style="margin-top:12px"><span class="label">Abandono de PIX</span>
      <span class="val"><span class="badge ${funil.abandono_pct>60?'err':funil.abandono_pct>30?'warn':'ok'}">${funil.abandono_pct}%</span></span></div>
  </div>

  <!-- Latência -->
  <div class="card">
    <h2>⚡ Velocidade de Respostas</h2>
    <div class="row"><span class="label">URL — P50</span><span>${lat_badge(lat.url?.p50,'url')}</span></div>
    <div class="row"><span class="label">URL — P95</span><span>${lat.url?.p95==null?'<span class=badge ok>—</span>':'<span class="badge '+(lat.url.p95<5000?'ok':lat.url.p95<10000?'warn':'err')+'">'+lat.url.p95+'ms</span>'}</span></div>
    <div class="row"><span class="label">PDF — P50</span><span>${lat_badge(lat.pdf?.p50,'pdf')}</span></div>
    <div class="row"><span class="label">PDF — P95</span><span>${lat.pdf?.p95==null?'<span class=badge ok>—</span>':'<span class="badge '+(lat.pdf.p95<8000?'ok':lat.pdf.p95<15000?'warn':'err')+'">'+lat.pdf.p95+'ms</span>'}</span></div>
    <div class="row"><span class="label">Token — P50</span><span>${lat_badge(lat.token?.p50,'token')}</span></div>
    <div class="row"><span class="label">Amostras (URL/PDF/Tkn)</span><span class="val" style="font-size:.85rem">${lat.url?.amostras||0} / ${lat.pdf?.amostras||0} / ${lat.token?.amostras||0}</span></div>
  </div>

  <!-- Erros -->
  <div class="card">
    <h2>🐛 Erros — Hoje</h2>
    <div class="row"><span class="label">URL timeout</span><span class="val">${err.url?.timeout||0}</span></div>
    <div class="row"><span class="label">URL parse_fail</span><span class="val">${err.url?.parse_fail||0}</span></div>
    <div class="row"><span class="label">PDF too_large</span><span class="val">${err.pdf?.too_large||0}</span></div>
    <div class="row"><span class="label">PDF parse_fail</span><span class="val">${err.pdf?.parse_fail||0}</span></div>
    <div class="row"><span class="label">PIX fail</span><span class="val">${err.pix?.fail||0}</span></div>
    <div class="row" style="font-weight:700">
      <span>Total de erros</span>
      <span class="badge ${err.total===0?'ok':err.total<10?'warn':'err'}">${err.total}</span>
    </div>
  </div>`;
...
async def painel_comercial(request: Request, _: None = Depends(verificar_sessao)):
